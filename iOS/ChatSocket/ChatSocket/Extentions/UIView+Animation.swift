...
public extension UIView {
    ...
    /**
     ...
     - Parameters:
     - endScale: end scale for the view, should be between 0 and 1
     - duration: duration of the animation, in seconds
     - delay: delay before the animation starts, in seconds
     - completion: block executed when the animation ends
     */
    @discardableResult func pushIn(endScale: CGFloat = 0.9,
                                   duration: TimeInterval = 0.3,
                                   delay: TimeInterval = 0,
                                   completion: ((Bool) -> Void)? = nil) -> UIView {
        
        
        UIView.animate(withDuration: duration/2, delay: delay, options: .curveEaseIn, animations: { () -> Void in
            self.transform = CGAffineTransform(scaleX: endScale, y: endScale)
        }, completion: { (completed: Bool) -> Void in
            UIView.animate(withDuration: duration/2, delay: 0.0, options: .curveEaseOut, animations: { () -> Void in
    ...
    @discardableResult func scale(endScale: CGFloat = 0.9,
                                  duration: TimeInterval = 0.3,
                                  delay: TimeInterval = 0,
                                  completion: ((Bool) -> Void)? = nil) -> UIView {
     ...
     */
    @discardableResult func swing(endScale: CGFloat = 0.9,
                                  duration: TimeInterval = 0.3,
                                  delay: TimeInterval = 0,
                                  completion: ((Bool) -> Void)? = nil) -> UIView {
